...
class BackInletScraperSpider(scrapy.Spider):
    name = 'back_inlet_scraper'
    allowed_domains = ['www.reece.com.au']
    start_urls = ['https://www.reece.com.au/search/toilets-c469/toilet-suites-c705/?sortBy=POPULARITY&sortDirection=DESCENDING&pageNumber=1&pageSize=45&generalFacets=waterinletposition%3Abackinlet']

    def parse(self, response):
        toilets = response.xpath('//div[@class="product-grid-wrapper"]/div')
        for toilet in toilets:
            toilet_url = toilet.xpath(
                './div[@class="product-tile__content"]/div[@class="product-tile__section-details"]/a/@href').extract_first()
            absoulte_toilet_url = response.urljoin(toilet_url)
            yield Request(absoulte_toilet_url, callback=self.toilet_parse)
